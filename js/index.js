...
function createHTML(result) {
  ...
  newestPostContainer.innerHTML = `<div class="newest"><h1>${result.title.rendered}</h1><div>${result.content.rendered}</div></div>`;
}
...
async function getFeaturedPosts() {
  try {
    ...
    for (let i = 0; i < results.length; i++) {
      ...
      featuredPostContainer.innerHTML += `<div class="featured"><h2>${results[i].title.rendered}</h2><p>${results[i].excerpt.rendered}</p><img class="" src="${results[i]._embedded["wp:featuredmedia"][0].media_details.sizes.medium.source_url}"/></div>`;
    }
  } catch (error) {
